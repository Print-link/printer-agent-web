...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://paper-link-backend-production.up.railway.app/api';
...
class ApiService {
  ...
  // Authentication
  async login(email: string, password: string, role?: 'manager' | 'clerk'): Promise<AuthResponse> {
    ...
    const user: User = {
      ...
      name: `${userData.firstName || ''} ${userData.lastName || ''}`.trim() || userData.email, // For backward compatibility
      ...
      createdAt: userData.createdAt ? (typeof userData.createdAt === 'string' ? new Date(userData.createdAt).getTime() : userData.createdAt) : Date.now(),
      updatedAt: userData.updatedAt ? (typeof userData.updatedAt === 'string' ? new Date(userData.updatedAt).getTime() : userData.updatedAt) : Date.now(),
    };
  ...
  async getClerkOrders(filters?: ClerkOrdersFilters): Promise<ClerkOrdersResponse> {
    const response = await this.axiosInstance.get('/clerk/orders', { params: filters });
    return response.data.data;
  }
  ...
  async getClerkOrdersByDate(date: string, branchId?: string): Promise<ClerkOrderDetail[]> {
    const params = branchId ? { date, branchId } : { date };
    const response = await this.axiosInstance.get('/clerk/orders/by-date', { params });
    return response.data.data;
  }

  async getClerkDashboardStats(branchId?: string): Promise<ClerkDashboardStats> {
    const params = branchId ? { branchId } : {};
    const response = await this.axiosInstance.get('/clerk/dashboard/stats', { params });
    return response.data.data;
  }

  async getClerkCategoryAnalytics(days?: number, branchId?: string): Promise<ClerkCategoryAnalytics[]> {
    const params: Record<string, string | number> = {};
    if (days) params.days = days;
    if (branchId) params.branchId = branchId;
    const response = await this.axiosInstance.get('/clerk/dashboard/category-analytics', { params });
    return response.data.data;
  }

  async getClerkWeeklyActivity(month?: string, year?: string, branchId?: string): Promise<ClerkWeeklyActivity[]> {
    const params: Record<string, string> = {};
    if (month) params.month = month;
    if (year) params.year = year;
    if (branchId) params.branchId = branchId;
    const response = await this.axiosInstance.get('/clerk/dashboard/weekly-activity', { params });
    return response.data.data;
  }
  ...
  async getManagerOverviewStats(filters?: {
    ...
    const response = await this.axiosInstance.get('/manager/dashboard/overview/stats', { params: filters });
    return response.data.data;
  }
  ...
  async getManagerOverviewBranches(filters?: {
    ...
    const response = await this.axiosInstance.get('/manager/dashboard/overview/branches', { params: filters });
    return response.data.data;
  }

  async getManagerOverviewCategoryAnalytics(days?: number): Promise<CategoryAnalytics[]> {
    const params = days ? { days } : {};
    const response = await this.axiosInstance.get('/manager/dashboard/overview/category-analytics', { params });
    return response.data.data;
  }

  async getManagerOverviewWeeklyActivity(month?: string, year?: string): Promise<WeeklyActivityItem[]> {
    const params: Record<string, string> = {};
    if (month) params.month = month;
    if (year) params.year = year;
    const response = await this.axiosInstance.get('/manager/dashboard/overview/weekly-activity', { params });
    return response.data.data;
  }
  ...
  async getManagerBranchStats(
    ...
    const response = await this.axiosInstance.get(`/manager/dashboard/branch/${branchId}/stats`, { params: filters });
    return response.data.data;
  }

  async getManagerBranchCategoryAnalytics(branchId: string, days?: number): Promise<CategoryAnalytics[]> {
    const params = days ? { days } : {};
    const response = await this.axiosInstance.get(`/manager/dashboard/branch/${branchId}/category-analytics`, { params });
    return response.data.data;
  }

  async getManagerBranchWeeklyActivity(branchId: string, month?: string, year?: string): Promise<WeeklyActivityItem[]> {
    const params: Record<string, string> = {};
    if (month) params.month = month;
    if (year) params.year = year;
    const response = await this.axiosInstance.get(`/manager/dashboard/branch/${branchId}/weekly-activity`, { params });
    return response.data.data;
  }

  // Branch APIs
  async getAllBranches(managerId: string): Promise<Branch[]> {
    const response = await this.axiosInstance.get('/branches', { params: { managerId } });
    return response.data.data;
  }
  ...
  async updateBranch(branchId: string, branchData: Partial<Omit<Branch, 'id' | 'createdAt' | 'updatedAt'>>): Promise<Branch> {
    const response = await this.axiosInstance.put(`/branches/${branchId}`, branchData);
    return response.data.data;
  }
  ...
  async getBranchOrderDetail(orderId: string, branchId?: string): Promise<ClerkOrderDetail> {
    ...
    const params = branchId ? { branchId } : {};
    const response = await this.axiosInstance.get(`/clerk/dashboard/orders/${orderId}`, { params });
    if (!response.data.success) {
  ...
  async updateBranchOrder(orderId: string, order: Partial<ClerkOrder>): Promise<ClerkOrderDetail> {
  ...
  async updateClerkOrder(orderId: string, order: Partial<ClerkOrder>): Promise<ClerkOrderDetail> {
  ...
  async createClerk(data: {
    ...
    const response = await this.axiosInstance.post('/auth/register/clerk', data);
  ...
  async changeClerkPassword(clerkId: string, newPassword: string): Promise<User> {
    ...
    const response = await this.axiosInstance.put(`/clerks/${clerkId}`, { password: newPassword });
  ...
  async setupPassword(password: string): Promise<User> {
    const response = await this.axiosInstance.post('/auth/setup-password', { password });
    return response.data.data.user;
  }

  // Category APIs
  async getAllCategories(adminId: string): Promise<Category[]> {
    const response = await this.axiosInstance.get('/categories', { params: { adminId } });
    return response.data.data;
  }
  ...
  async getAgentServices(branchId: string, isActive?: boolean): Promise<AgentService[]> {
    const params: Record<string, string | boolean> = { branchId };
    if (isActive !== undefined) {
      params.isActive = isActive;
    }
    const response = await this.axiosInstance.get('/agent-services', { params });
    return response.data.data || response.data;
  }

  async getAgentServiceById(agentServiceId: string): Promise<AgentService> {
    const response = await this.axiosInstance.get(`/agent-services/${agentServiceId}`);
    return response.data.data || response.data;
  }

  async createAgentService(data: CreateAgentServiceData): Promise<AgentService> {
    const response = await this.axiosInstance.post('/agent-services', data);
    return response.data.data || response.data;
  }

  async updateAgentService(agentServiceId: string, data: UpdateAgentServiceData): Promise<AgentService> {
    const response = await this.axiosInstance.put(`/agent-services/${agentServiceId}`, data);
    return response.data.data || response.data;
  }
