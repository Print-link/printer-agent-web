...
import type { BranchCategoryBreakdown, CategoryAnalytics } from "../../types";
...
export default function OverviewPage() {
	...
	const branchChartData = useMemo(() => {
		return branchBreakdown.map((branch: BranchCategoryBreakdown) => ({
			name:
				branch.branchName.length > 15
					? branch.branchName.substring(0, 15) + "..."
					: branch.branchName,
			revenue: (branch.revenue as unknown as { revenue: number }) || 0,
			orders: (branch.orders as unknown as { orders: number }) || 0,
		}));
	}, [branchBreakdown]);
			...
			{categoryAnalytics.length > 0 && (
				...
				>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{categoryAnalytics
							.slice(0, 6)
							.map((category: CategoryAnalytics, index: number) => (
								<div
