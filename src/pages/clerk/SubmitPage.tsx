...
export default function SubmitPage() {
	...
	const submitMutation = useMutation({
		mutationFn: async (
			...
			const job = await apiService.createJob({
				...
				customerName: uploadResult.fileName,
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		if (!selectedFile || !printer) {
			setSubmitError(
				!selectedFile ? "Please select a file" : "Please select a printer"
			);
			return;
		}
		...
		submitMutation.mutate({
			// Pass optional PrintJob fields if needed
			printerId: printer || undefined,
		});
	};
	...
	return (
		...
		>
			...
			>
				...
				{submitError && (
					<div
						className={`mb-6 p-4 rounded-md flex items-center justify-between gap-3 ${
							theme === "dark"
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div>
						...
						{selectedFile ? (
							<div
								className={`p-4 rounded-lg border-2 border-amber-400 flex items-center gap-4 ${
