...
export default function DashboardPage() {
  ...
  // Use appropriate dashboard hook based on role
  const clerkDashboard = isClerk ? useClerkDashboard() : null;
  const managerOverview = isManager && !selectedBranch ? useManagerOverviewDashboard() : null;
  const managerBranch = isManager && selectedBranch ? useManagerBranchDashboard(selectedBranch.id) : null;
    ...
    return (
      <div className={`p-6 rounded-lg ${
        ...
        <p className={theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}>
          You are not assigned to a branch. Please contact your manager to assign you to a branch.
        </p>
      </div>
    );
  }
  ...
  if (hasError && !isLoading && !stats) {
    return (
      <div className={`p-6 rounded-lg ${
        ...
        <div className={`p-8 text-center border-2 rounded-lg ${
          theme === 'dark' 
  ...
  return (
    <div className="flex flex-col gap-3 h-full p-0">
      <div className="flex flex-col gap-3">
        ...
        {isManager && (managerOverview || managerBranch) && (
          <ManagerCategoryAnalytics
            categoryAnalytics={(managerOverview || managerBranch)!.categoryAnalytics}
            isLoading={isLoading}
        ...
        {/* Orders by Date List */}
        {isClerk && clerkDashboard && selectedDate && (
          <OrdersByDateList
            orders={clerkDashboard.ordersByDate}
            selectedDate={selectedDate}
            isLoading={isLoading}
          />
        )}
