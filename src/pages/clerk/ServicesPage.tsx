...
type ViewState = 'categories' | 'subcategories' | 'templates' | 'agent-services';
...
export default function ServicesPage() {
  ...
  const [navState, setNavState] = useState<NavigationState>({ view: 'categories' });
  ...
  const loadAgentServices = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load agent services');
    } finally {
      setIsLoading(false);
    }
  }, [selectedBranch?.id]);
  ...
  const loadServiceTemplates = async (subCategoryId: string) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load service templates');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleTemplateSelect = (template: ServiceTemplate) => {
    ...
    // Show form to create agent service for this template
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            <div>
              <h1 className={`text-2xl font-semibold m-0 mb-1 ${
        ...
        {/* Agent Service Form Modal */}
        {showServiceForm && (
          <AgentServiceForm
            isOpen={showServiceForm}
            branchId={selectedBranch.id}
            template={navState.selectedTemplate || editingService?.serviceTemplate}
            editingService={editingService}
            subCategory={navState.selectedSubCategory || editingService?.subCategory}
            onClose={() => {
              setShowServiceForm(false);
              setEditingService(null);
            }}
