...
export default function SetupLocationPage() {
	...
	const [address, setAddress] = useState(
		(user as unknown as { location?: { address: string } }).location?.address ||
			""
	);
	const [latitude, setLatitude] = useState(
		(user as unknown as { location?: { latitude: number } }).location
			?.latitude || 0
	);
	const [longitude, setLongitude] = useState(
		(user as unknown as { location?: { longitude: number } }).location
			?.longitude || 0
	);
